...
void MeshLib::CHarmonicMap::_calculate_edge_weight() 
{
    ...
    for (M::MeshFaceIterator fiter(m_pMesh); !fiter.end(); ++fiter)
    {
		...
		//_inverse_cosine_law
		int i = 0;
		for (M::FaceHalfedgeIterator hEiter(pF); !hEiter.end(); ++hEiter)
		{
			pH[i++] = *hEiter;
		}
		...
		pH[0]->angle() = this->_inverse_cosine_law(a, b, c);
		pH[1]->angle() = this->_inverse_cosine_law(b, c, a);
		pH[2]->angle() = this->_inverse_cosine_law(c, a, b);
		//std::cout << pH[0]->angle() << " " << pH[1]->angle() << " " << pH[2]->angle() << std::endl;
    }
    ...
    for (M::MeshEdgeIterator eiter(m_pMesh); !eiter.end(); ++eiter)
    {
		...
		if (pE->boundary()) {
			double a = reinterpret_cast<M::CHalfEdge*>(pE->halfedge(0)->he_prev())->angle();
			double w = 1.0f / std::tan(a);
			pE->weight() = w;
		}
		else {
			double a = reinterpret_cast<M::CHalfEdge*>(pE->halfedge(0)->he_prev())->angle();
			double b = reinterpret_cast<M::CHalfEdge*>(pE->halfedge(1)->he_prev())->angle();
			double w = 1.0f / std::tan(a) + 1.0f / std::tan(b);
			pE->weight() = w;
		}
		//std::cout << pE->weight() << std::endl;
    }
}
