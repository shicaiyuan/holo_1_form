...
double MeshLib::CSphericalHarmonicMap::step_one(int steps, double step_length)
{
    ...
    for (int i = 0; i < steps; ++i)
    {
        for (M::MeshVertexIterator viter(m_pMesh); !viter.end(); ++viter)
        {
			...
			CPoint laplacian = CPoint(0,0,0);
			for (M::VertexVertexIterator vviter(pV); !vviter.end(); ++vviter) {
				M::CVertex *pv = *vviter;
				double weight = m_pMesh->vertexEdge(pv, pV)->weight();
				CPoint delta = pv->u() - pV->u();
				laplacian += CPoint(delta[0]* weight, delta[1]* weight, delta[2]* weight);
			}
			
            // 2. get the noraml component
			double value = laplacian * pV->u();
			CPoint normalComp = CPoint(value * pV->u()[0], value * pV->u()[1], value * pV->u()[2]);
            // 3. get the tangent_component
			CPoint tangentComp = laplacian - normalComp;
            // 4. update u
			pV->u() += tangentComp * step_length;
            // 5. normalize the vertex->u() to the unit sphere
			double length = pV->u().norm();
			pV->u() = pV->u() / length;
...
void MeshLib::CSphericalHarmonicMap::_normalize()
{
	...
	CPoint numerator = CPoint(0, 0, 0);
	for (M::MeshVertexIterator viter(m_pMesh); !viter.end(); ++viter) {
		M::CVertex* pv = *viter;
		area += pv->area();
		numerator += pv->u() * pv->area();
	}
	center = numerator / area;
	for (M::MeshVertexIterator viter(m_pMesh); !viter.end(); ++viter)
	{
		M::CVertex* vP = *viter;
		vP->u() = (vP->u() - center) / (vP->u() - center).norm();
	}
}