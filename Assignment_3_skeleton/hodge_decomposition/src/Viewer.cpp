...
void initializeBmpTexture()
{
	...
	int ImageWidth = g_image.cols;
	int ImageHeight = g_image.rows;
	GLubyte* ptr = (GLubyte*)g_image.data;
...
void display()
{
	...
	if (cap.isOpened()) {
		cap >> g_image;
		if (g_image.data != NULL) {
			cv::cvtColor(g_image, g_image, cv::COLOR_BGR2RGB);
			glBindTexture(GL_TEXTURE_2D, g_texture_name);
			// glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
			// glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
